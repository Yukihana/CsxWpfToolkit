...
public class EnumGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // This combines the compilation data into a cache to prevent generating every single time.
        // Combine the selected enums with the `Compilation`
        // Do not pass Syntax types into Combine
        // It does not implement equality.
        // And it will keep getting added to cache everytime.
        // Create an equatable model first, (aka records and Equatable array only)
    ...
    private static EnumDeclarationSyntax? GetSemanticTargetForGeneration(GeneratorSyntaxContext context)
    {
        ...
        foreach (AttributeListSyntax attributeListSyntax in enumDeclarationSyntax.AttributeLists)
        {
            foreach (AttributeSyntax attributeSyntax in attributeListSyntax.Attributes)
            {
                ...
                if (fullName == "CSX.Toolkits.WpfGenerators.EnumExtensionsAttribute")
                {
                    // return the enum
    ...
    private static List<EnumToGenerate> GetTypesToGenerate(Compilation compilation, IEnumerable<EnumDeclarationSyntax> enums, CancellationToken ct)
    {
        ...
        INamedTypeSymbol? enumAttribute = compilation.GetTypeByMetadataName("CSX.Toolkits.WpfGenerators.EnumExtensionsAttribute");
